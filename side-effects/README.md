...
```js// the best way to avoid side-effects is to copy arguments
...
```js// modify these functions so to avoid side-effects
...
```js// there are many ways to copy arrays & objects in javascript
...
```js// arrays and objects arguments are passed by reference
